...
ДЕ-КОНКРЕТИЗАЦИЯ:
При создании статьи УБИРАЙ конкретные детали, не имеющие ценности для базы знаний:
- Конкретные даты, дни недели, время записи
- Адреса и филиалы клиники
- Имена клиентов и питомцев
- Номера телефонов
ОСТАВЛЯЙ: симптомы, диагнозы, назначения, цены, рекомендации.
trigger_phrases: убирай из них конкретные даты/время, оставляй только суть обращения.
evidence.quote: только медицинская суть, без организационных деталей.
